...
namespace LinqExamples
{
    ...
    public class Topic
    {
        ...
        public override string ToString()
        {
            var result = $"{Id,2}) {TopicName,25}";
            return result;
        }

        public static int TopicNameToId (string x)
        {
            ...
            return topics.Where(s => s.TopicName.Equals(x)).Select(s => s.Id).FirstOrDefault();
        }
    }
    ...
    public class Student
    {
        ...
        public static List<Student> FromSWTtoS(List<StudentWithTopics> students)
        {
            return Generator.GenerateStudentsWithTopicsEasy().Select(x => new Student
            (
                x.Id,
                x.Index,
                x.Name,
                ...
                x.Active,
                x.DepartmentId,
                x.Topics.ConvertAll(x => Topic.TopicNameToId(x))
            )).ToList();
        }
    }
    ...
    public class Program
    {
        ...
        static void Z1(int n)
        {
            ...
            var groupedStud = resStud.Select((x, i) => new { Index = i, Value = x })
                                     .GroupBy(x => x.Index / n)
                                     .Select(x => x.Select(v => v.Value).ToList())
                                     .ToList();
            
            foreach (var x in groupedStud)
            {
        ...
        static void Z2a()
        {
            var resStud = from s in Generator.GenerateStudentsWithTopicsEasy().SelectMany(s => s.Topics)
                      group s by s into g
        ...
        static void Z2b()
        {
            var temp = Generator.GenerateStudentsWithTopicsEasy()
                         .SelectMany(x => x.Topics, (x, y) => new { gender = x.Gender, Topic = y })
                         .GroupBy(x => (x.gender, x.Topic))
                         .Select(x => new {x.Key.gender, x.Key.Topic, Count = x.Count()})
                         .OrderBy(x => x.gender)
                         .ThenByDescending(x => x.Count)
                         .ToList();
            var result = from s in temp
                       group s by s.gender into sGroup
                       select new
                       {
                           Gender = sGroup.Key,
                           Topics = sGroup.Select(s => new { s.Topic, Count = s.Count })
                       };
        ...
        static void Z3()
        {
            var students = Student.FromSWTtoS(Generator.GenerateStudentsWithTopicsEasy());
        ...
        static void Z4a()
        {
            ...
            var uczen = assem.CreateInstance("LinqExamples.Student");
            Type info = uczen.GetType();
            ...
            foreach (var meth in meths)
            {
                ...
                Console.WriteLine($" Params amount : {meth.GetParameters().Length}");
            }
        }

        static void Z4b()
        {
            Topic topic = new Topic(50, "C#");
            MethodInfo methodInfo = topic.GetType().GetMethod("TopicNameToId", new Type[] { typeof(string) });
            int topicId = (int)methodInfo.Invoke(topic, new object[] { topic.TopicName });
            Console.WriteLine($"Topic id for topic {topic.TopicName} is {topicId}");
        ...
        static void Main()
        {
            ...
            var resStud = Generator.GenerateStudentsWithTopicsEasy().ToList();
